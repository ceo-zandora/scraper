...
def csvMaker(cdate):
    with open('Data/' + cdate + '.csv','w',newline='',errors='ignore') as f:
        wr = csv.writer(f)
        wr.writerow([
            'ID',
            'NAME',
...
cdate = datetime.now().strftime('%d_%h_%H_%M')
csvMaker(cdate)

def csvWriter(data,cdate):
    with open('Data/' + cdate + '.csv', 'r') as file:
...
def Scrape(detail, retries = 3):
    ...
    if detail['WEBSITE'] == "":
        ...
        if clean_email != "":
            ...
            csvWriter(data=data,cdate=cdate)
        else:
            Email = detail['EMAIL']
